...
each other. The game has different rules for each type of fighter. We are going to focus on two
specific types: Wizards and Warriors.
...
5. **Create the Wizard class**\
   Create another new class called Wizard. This class should also inherit from the existing Fighter
   class.
6. **Describe a Wizard**\
...
7. **Allow Wizards to prepare a spell and make them vulnerable when not having prepared a spell**\
   Update the Wizard class to add a method called prepareSpell(). The class should remember when
   this method is called, and make sure that its isVulnerable() method returns false only when a
   spell is prepared.
